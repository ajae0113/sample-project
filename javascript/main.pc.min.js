...
this.stage.css({bottom:"0",opacity:"0",display:"none"});this.initColorCss()},initColorCss:function(){this.stage.css({color:this.fontColor})},initDefaultDom:function(){var a=this;this.closeUrl=form_icons.close?"data:image/png;base64,"+form_icons.close:uiBaseURL+"close.png";this.close=$('\n\t\t\t<button class="close aria-label-button" aria-label="Close '+FormInfo.getFormTitle(this.formType)+'">\n\t\t\t\t<img src="'+this.closeUrl+'" alt="" aria-hidden="true">\n\t\t\t</button>\n\t\t');this.close.on(_event._click,
function(){return a.hide()});this.close.find("img").changeButtonColor(this.fontColor);this.leftBtn=$('\n\t\t\t<button class="leftBtn btn aria-label-button" aria-label="Previous Page">\n\t\t\t\t<img src="'+this.leftButtonUrl+'" alt="" aria-hidden="true">\n\t\t\t</button>\n\t\t');this.rightBtn=$('\n\t\t\t<button class="rightBtn btn aria-label-button" aria-label="Next Page">\n\t\t\t\t<img src="'+this.rightButtonUrl+'" alt="" aria-hidden="true">\n\t\t\t</button>\n\t\t');this.initSwiper()},initSwiper:function(){this.thumbnailSwiper=
...
this.edtPageNumber=$("\n      <div class='pageNumber'>\n        <label for='currentPageIndexTextField' class='sr-only'>\n          "+getLanguage("ariaPageNumber","page number")+"\n        </label>\n        <input \n          aria-label='"+getLanguage("ariaPageNumber","page number")+"' \n          id='currentPageIndexTextField' \n          autocomplete='off' \n          size='5' \n          value='' \n        />\n      </div>\n    ");this.parent.append(this.logoBar);this.parent.append(this.topRightBar);
bookConfig.appLogoIcon||this.logoBar.css({display:"none",height:"0"});!bookConfig.aboutButtonVisible&&window.userInfo&&0<window.userInfo.c&&this.topRightBar.css({display:"none",height:"0"});this.bottomBar.append(this.buttonBar);this.bottomBar.append(this.bottomLeftBar).append(this.bottomRightBar);this.bottomLeftBar.append(this.edtPageNumber);this.parent.append(this.bottomBar);this.initPageNumber()},showOrHideLogoBar:function(a){bookConfig.appLogoIcon||(a=!1);a?this.logoBar&&this.logoBar.css({display:"block"}):
this.logoBar&&this.logoBar.css({display:"none"})},initPageNumber:function(){var a=this.edtPageNumber.find("input"),b=parseInt(bookConfig.totalPageCount),c=16;""==bookConfig.pageNumberCaption&&(1E4<=b&&(c=16),1E4>b&&(c=10),1E3>b&&(c=7),100>b&&(c=4),10>b&&(c=3));a.attr("size",c);Color(this.color.add(120));a.css({color:bookConfig.iconColor,background:"transparent",border:"none"});this.edtPageWidth=this.edtPageNumber.width()},initExtendedbar:function(){this.extendedBar=new PCExtendedBar(this.parent);
global.moreBar=this.extendedBar},resetToolbarColor:function(){this.color=Color(bookConfig.toolbarColor);this.topBarAlpha=Math.min(bookConfig.topToolBarAlpha,bookConfig.ToolBarAlpha);this.extendedBar&&this.extendedBar.$bar.css({"background-color":this.color.rgba(this.topBarAlpha)});this.resetButtonTitleColor();global.PcTelephoneNumberFrame&&global.PcTelephoneNumberFrame.setColor({color:bookConfig.toolbarColor,textColor:bookConfig.iconFontColor})},setButtonTitle:function(a){if("WeChatShareButton"==
a.getClassName())global.WeChatShareImg=new WeChatShare({parent:this.parent,color:"#fff",textColor:bookConfig.toolbarColor,margin:10,padding:10,arrowHeight:12,arrowWidth:12});else if("TelephoneNumberButton"==a.getClassName())global.PcTelephoneNumberFrame=new PcTelephoneNumberBubble({parent:this.parent,color:this.color,textColor:a.fontColor,margin:4,padding:15,arrowHeight:8,arrowWidth:8});else{var b=new TextBubble({parent:this.parent,text:a.caption,direction:TextBubble.TOP,color:this.color,textColor:a.fontColor,
margin:7});a.setTitle(b)}},addTopRightButton:function(){this.topRightButton||(this.topRightButton=$("<div class='topRightButton' id='createButton'></div>"),bookConfig.ToolBarVisible?(this.topRightBar.append(this.topRightButton),this.onResize()):this.parent.append(this.topRightButton))},removeTopRightButton:function(){this.topRightButton&&(this.topRightButton.remove(),this.topRightButton=null)},onResize:function(a,b){var c=this,d,e,f,g;return $jscomp.asyncExecutePromiseGeneratorProgram(function(h){if(1==
h.nextAddress)return bookConfig.ToolBarVisible?h.yield(new Promise(function(k){return requestAnimationFrame(k)}),2):h.return();a=void 0==a?c.parent.width():parseInt(a);b=void 0==b?c.parent.height():parseInt(b);d=a/4;e=a/4;f=a-d-e;g=c.edtPageWidth+150+20;f<g&&(f=g,d=(a-f)/2);c.buttonBarWidth=f;c.resetButtonPosition();h.jumpToEnd()})},afterButtonClick:function(a){this.extendedBar.visible&&"btnSocialShare"!=a.language.key&&(a!=this.btnMore?isPhone()||isPad()||this.extendedBar.hide():(a=this.buttonsInExtendedBar(),
a.each(function(b){b.changeColor()}),a=null))},resetButtonPosition:function(){(new PCButtonQueue({buttons:this.buttons,buttonWidth:30,buttonTop:5,color:this.color,bottomBar:{leftBar:this.bottomLeftBar,rightBar:this.bottomRightBar},container:{body:this.buttonBar,topRightBar:this.topRightBar,width:this.buttonBarWidth},extended:this.extendedBar.$bar,edtNumber:{body:this.edtPageNumber,width:this.edtPageWidth,top:8,space:5}})).sort();this.extendedBar.resize(this.buttonsInExtendedBar());this.extendedBar.hide();
this.buttons.each(function(a){a.changeColor()}.bind(this))}}).extend("Toolbar");
